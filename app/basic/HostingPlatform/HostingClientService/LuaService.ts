  ...
  private async runLuaScript(): Promise<void> {
    ...
    if (errorLines) {
      errorLines.forEach(errorLineStr => {
        ...
        let errorLine = Number(errorLineStr.substr(1, errorLineStr.length - 2));
        ...
        if (this.luaVm) {
          errorLine -= this.luaVm.getLuaBuidinCodeLines() - 1;
        }
        ...
        for (const i in offset) {
          if (curLines + offset[i].offset > errorLine) {
            this.logger.warn(`Lua error may be occurred in ${offset[i].compName} line ${errorLine - curLines}`);
            return;
          }
