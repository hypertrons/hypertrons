...
import LabelSetupConfig from '../../../component/label-setup/config';
...
export class LuaService<TConfig extends HostingConfigBase, TRawClient> extends ClientServiceBase<TConfig, TRawClient> {
  ...
  protected lua_on(eventType: string, cb: (e: any) => void) {
    ...
    const func = async e => {
      if (e.fullName !== this.client.getFullName() || e.installationId !== this.client.getHostId()) {
  ...
  protected async lua_labelSetup() {
    // config check
    const labelConfig = this.client.getCompConfig<LabelSetupConfig>('label-setup');
